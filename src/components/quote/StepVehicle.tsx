...
import { Car, ChevronLeft, ChevronRight, Info, KeyRound, PencilLine, CheckCircle2 } from "lucide-react";
...
export function StepVeiculo({
  ...
  const submitPlaca = async (e: React.FormEvent) => {
    ...
    // Antes de avançar, mostra os dados encontrados para o cliente confirmar.
    setConfirming(true);
  };
    ...
    return (
      <div className="flex flex-col gap-5">
        ...
        <div className="rounded-2xl border bg-muted/20 p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <p className="text-sm font-semibold text-foreground">{data.marca || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Modelo</p>
              <p className="text-sm font-semibold text-foreground">{data.modelo || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Ano de fabricação</p>
              <p className="text-sm font-semibold text-foreground">{data.ano_fab || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Ano do modelo</p>
              <p className="text-sm font-semibold text-foreground">{data.ano_mod || "—"}</p>
            </div>
          </div>
        </div>
        ...
        <p className="flex items-start gap-1.5 text-xs text-muted-foreground">
          ...
          Confira se esses são os dados corretos do seu veículo antes de continuar.
        </p>
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row">
          <button
            ...
            className="inline-flex h-12 items-center justify-center gap-1 rounded-xl border bg-background px-4 text-sm font-medium text-foreground transition hover:bg-accent sm:w-1/2"
          >
      ...
      <form onSubmit={submitPlaca} className="flex flex-col gap-5">
        ...
        <div>
          ...
          <button
            ...
            className="mt-1.5 inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground underline-offset-2 hover:text-brand hover:underline"
          >
